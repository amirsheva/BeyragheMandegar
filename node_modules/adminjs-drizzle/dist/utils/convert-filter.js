...
export function convertFilter(filter) {
    ...
    for (const filter of Object.values(filters)) {
        ...
        if (['boolean', 'number', 'float', 'object', 'array'].includes(property.type())) {
            condition = eq(property.column, safeParseJSON(filter.value));
        }
            ...
            if (filter.value.from && filter.value.to) {
                condition = and(gte(property.column, new Date(filter.value.from)), lte(property.column, new Date(filter.value.to)));
            }
